...
public class Conectar {
    ...
    public Boolean Logar(String telefone) {
        this.email = email;
        this.telefone = telefone;

        Monitoramento monitoramento = new Monitoramento();
        Boolean login = conectarSQL.Logar(email, telefone);

        String sqlSelect = "SELECT emailCorporativo, telefone FROM Funcionario;";
        if (login == true){
            System.out.println("Sessao encerrada\n\n");
        } else {
            try {
                ...
                while (resposta.next()) {
                    ...
                    if (this.email.equals(username) && this.telefone.equals(telefone)) {
                        logado = true;
                        System.out.println("""
                                Login Realizado com Sucesso!
